...
def upgrade() -> None:
    op.add_column(
        "legacies", sa.Column("background_image_id", sa.UUID(), nullable=True)
    )
    op.create_index(
        op.f("ix_legacies_background_image_id"),
        "legacies",
        ["background_image_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_constraint(
        "fk_legacies_background_image_id", "legacies", type_="foreignkey"
    )
