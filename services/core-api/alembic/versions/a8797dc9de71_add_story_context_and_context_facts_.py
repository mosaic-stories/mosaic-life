...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_story_contexts_story_id"), "story_contexts", ["story_id"], unique=False
    )
    op.create_index(
        op.f("ix_story_contexts_user_id"), "story_contexts", ["user_id"], unique=False
    )
    op.create_table(
        "context_facts",
        sa.Column("id", sa.UUID(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["story_context_id"], ["story_contexts.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_context_facts_story_context_id"),
        "context_facts",
        ["story_context_id"],
        unique=False,
    )
