...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "details", JSONB(), server_default=sa.text("'{}'::jsonb"), nullable=False
        ),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "ix_knowledge_audit_log_legacy_id", "knowledge_audit_log", ["legacy_id"]
    )
    op.create_index(
        "ix_knowledge_audit_log_created_at", "knowledge_audit_log", ["created_at"]
    )
    op.create_index(
        "ix_knowledge_audit_log_story_id", "knowledge_audit_log", ["story_id"]
    )
