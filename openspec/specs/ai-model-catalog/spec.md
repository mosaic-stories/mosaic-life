...
The LiteLLM proxy's alias-based Bedrock model catalog: how a Bedrock-backed
model becomes callable by core-api and other internal clients through a
stable alias, and what must stay true (config-only extension, catalog
visibility via `/v1/models`, cross-environment consistency) as new aliases
are added.
