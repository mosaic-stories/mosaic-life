...
- [ ] 3.1 After merge, confirm ArgoCD synced `infra/helm/litellm` and the pod is running the new image tag (`kubectl -n aiservices get pods -o jsonpath` or ArgoCD UI — inspection only, per CLAUDE.md's GitOps rules).
- [ ] 3.2 Hit `/health/liveliness` and `/health/readiness` on the deployed proxy and confirm 200s.
- [ ] 3.3 Call `/v1/models` and confirm all 10 pre-existing aliases are still listed.
- [ ] 3.4 Run one live completion through core-api's LiteLLM adapter against an existing alias (e.g. `claude-sonnet-4-6`) and confirm a successful response end-to-end.
- [ ] 3.5 Confirm Prometheus is still scraping `/metrics` on the proxy and that existing dashboard queries aren't broken by the new `service_tier` label.
- [ ] 3.6 If any of 3.2–3.5 fail, revert the Phase 1 PR (single-file `values.yaml` revert) before proceeding to Phase 2.

## 4. Phase 2 — Add the new Bedrock model catalog entries

- [ ] 4.1 In `infra/helm/litellm/templates/configmap.yaml`, add `claude-sonnet-5` (`bedrock/us.anthropic.claude-sonnet-5`), `claude-opus-5` (`bedrock/us.anthropic.claude-opus-5`), and `glm-5` (`bedrock/zai.glm-5`) to `model_list`, grouped/commented consistently with the existing entries.
...
- [ ] 4.3 Update `docs/ai-models.md` model tables to include the three new aliases and their IDs per environment.
- [ ] 4.4 Commit and open a PR scoped to Phase 2 only.

## 5. Phase 2 — Local and staging verification

- [ ] 5.1 Locally: `docker compose -f infra/compose/docker-compose.yml up -d litellm` (or full stack), then `curl http://localhost:14000/v1/models` and confirm the three new aliases are listed.
- [ ] 5.2 Locally: run one completion against each of `claude-sonnet-5`, `claude-opus-5`, and `glm-5` through the local LiteLLM proxy and confirm successful responses (or a clear, expected `AccessDeniedException` if 1.2's Bedrock access precondition hasn't landed yet in this AWS account — distinguish that from a config bug).
